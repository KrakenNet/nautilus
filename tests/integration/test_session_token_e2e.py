...
def test_ac_18_a_b_issue_then_verify_round_trip() -> None:
    service = SessionTokenService(key_ring=KeyRing(), broker_instance_id="broker-int")
    token = service.issue(session_id="s", agent_id="a", purpose="p", clearance="c")
...
def test_ac_18_d_broker_instance_mismatch_is_rejected() -> None:
    ...
    token = issuer.issue(session_id="s", agent_id="a", purpose="p", clearance="c")
