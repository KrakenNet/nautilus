...
def test_w72_a_shipped_pack_scopes_on_a_field_the_source_actually_has(
    pg_dsn: str, write_config: Any, monkeypatch: pytest.MonkeyPatch, pack: str
) -> None:
    ...
    operator's own table. No table has a ``purpose`` column, so the request
    fails with ``UndefinedColumnError``.

    A load-time blocker became a run-time one. Either the constraint names a
    real column, or the pack refuses to load; shipping a compliance pack that
    turns every covered request into a backend error is worse than shipping none.
    """
    monkeypatch.setenv("WAVE7_PG_DSN", pg_dsn)
    config = write_config(
        {
            "sources": [
                _pg_source(data_types=["phi", "pii", "patients"], classification="confidential")
            ],
    ...
    assert not response.sources_errored, (
        ...
        f"{response.sources_errored}. The pack asserts scope_constraint "
        f"field='purpose'; the adapter emits WHERE purpose = ... against the "
        f"user's table. rule_packs/*/rules/{{minimum-necessary,ac-6-least-privilege}}.yaml"
    )
    assert response.data.get("src"), (
        f"pack {pack!r} routed the source but returned no rows and reported no "
        f"error -- a silent empty result is the other half of the same defect"
    )
...
def test_w73_es_mapping_cache_sees_a_field_the_index_gained_later(
    ...
    ``execute`` fetches the mapping once (``if self._props is None``) and never
    refetches. ``_exact_field`` treats a field missing from that cache as
    ...
    warm = _scoped(config, "classification", "!=", "top secret")
    ...
    httpx.put(
        ...
        json={"title": "cafeteria roster", "classification": "public", "owner": "catering dept"},
        timeout=30,
    ).raise_for_status()
    httpx.post(f"{es_two_stage}/wave7-docs/_refresh", timeout=30).raise_for_status()

    response = _scoped(config, "owner", "!=", "finance dept")
...
def test_w73_es_range_operators_run_against_the_exact_field(
    ...
    ``_EXACT_OPERATORS`` excludes ``<``/``>``/``<=``/``>=``/``BETWEEN`` with the
    comment "they are already lexicographic". They are -- over the *analysed
    terms*. On a dynamically-mapped string, ``classification >= 'public'``
    compares against the tokens ``top``/``secret``/``public``, so 'top secret'
    ranks by 'secret', and the answer is wrong in a way no error reports.
    """
    monkeypatch.setenv("WAVE7_ES_URL", es_two_stage)
    config = _es_config(write_config, "wave7-docs")
...
def test_w73_influx_intersects_two_time_bounds(
    ...
    now_ns = time.time_ns()
    lines = "\n".join(
        f"cpu,host=web-{i} usage={i}i {now_ns - i * 3600 * 1_000_000_000}" for i in range(6)
    )
    httpx.post(
        f"{url}/api/v2/write",
...
def test_w76_an_upgrade_does_not_quarantine_every_existing_source(
    ...
    async def _run() -> Any:
        broker = Broker.from_config(config)
        # The shape 0.2.x wrote: a pre-narrowing fingerprint, no format tag.
        broker.fingerprint_store.record("src", "sha256:" + "0" * 64)
        try:
...
def test_w77_mcp_http_does_not_share_one_session_across_callers(
    ...
    ``mcp_server.py`` builds the server with ``stateless_http=True``, so
    ``ctx.session_id`` is not a per-client value; two unrelated agents land on
    the same session id and read each other's cumulative exposure. Either the
    transport carries a real session, or it refuses to start when session state
    is configured -- silently pooling two callers' ledgers is the failure the
    session store exists to prevent.
    """
    monkeypatch.setenv("WAVE7_PG_DSN", pg_dsn)
    config = write_config(
        {
    ...
    try:
        ...
        stateless = getattr(getattr(server, "settings", None), "stateless_http", None)
        if stateless is None:
            stateless = getattr(server, "_stateless_http", None)
        assert stateless is not True, (
            "the MCP server is built with stateless_http=True while a session "
            "store is configured, so ctx.session_id is not per-client and two "
            "agents share one cumulative-exposure ledger (mcp_server.py). Bind "
            "the session id to the transport session, or refuse the transport."
        )
    finally:
        broker.close()
...
def test_w79_a_token_minted_by_one_process_verifies_in_another() -> None:
    """Session tokens must verify across replicas, or they are not a protocol.

    ``KeyRing()`` generates its own signing key in its constructor and takes no
    persistence argument, so every broker process holds a different key. A token
    minted by replica A is rejected by replica B with ``unknown_kid``. Every
    ...
    The control is the same-ring verification below: if that fails, the fixture
    is wrong and the cross-ring failure says nothing.
    """
    ...
    replica_a = SessionTokenService(key_ring=KeyRing(), broker_instance_id="nautilus")
    ...
    replica_b = SessionTokenService(key_ring=KeyRing(), broker_instance_id="nautilus")
    try:
        claims = replica_b.verify(token)
    except SessionTokenError as exc:
        pytest.fail(
            f"a token minted by one broker process was rejected by another "
            f"({exc.reason_code}). KeyRing material has to be shared or persisted "
            f"for a multi-replica deployment; today each process generates its own "
