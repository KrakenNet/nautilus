...
Files in `deploy/` in the repository:
...
# What the manifests in `deploy/` need: configmap.yaml declares a
...
| Healthy | `200 {"status":"ok","version":"…","build":"…"}` — see [`GET /healthz`](../reference/rest-api.md#get-healthz) for what the two identifiers mean | `200 {"status":"ok"}` |
...
[Running more than one replica](operator-guide.md#running-more-than-one-replica).
...
([which keys, and why](operator-guide.md#which-keys-reload-and-which-need-a-restart)):
...
| `Application startup failed. Exiting.` preceded by `PostgresSessionStore unavailable (dsn=postgresql://postgres:5432): [Errno -2] Name or service not known`, then `ERROR: application startup failed; the server never accepted a connection. The cause is logged above.` | `on_failure: fail_closed` and the database is unreachable | fix DNS/credentials/network policy. That `dsn=` is the `scheme://host[:port]` `redact_connection` copies out by allowlist — no password, and no database name either, so read those from `session_store.dsn`. Exiting non-zero here is deliberate: the alternative was a pod that exited `Completed` and never restarted |
...
hashes say; see
[Back up](operator-guide.md#back-up) for why the `-wal` matters.
