...
**HTTP 401**, with `WWW-Authenticate: Bearer`. `nautilus/transport/fastapi_app.py:729-733`. The
...
`nautilus/core/broker.py:3634-3635`. `context["session_token"]` was present but not a `str`
...
`nautilus/core/broker.py:3642-3647`. The token is valid, but the request's `agent_id` is not the
...
`.revoke_signing_key` (`nautilus/core/broker.py:1893`, `:1864`, `:1905`, `:1939`).
...
`nautilus/core/broker.py:1899-1902`. The requested `purpose` is not in that agent's
...
- `handoff requires the originating agent's session token` — `nautilus/core/broker.py:2477-2481`.
- `session token rejected: {exc.reason_code}` — `nautilus/core/broker.py:2500-2504`, using the same
