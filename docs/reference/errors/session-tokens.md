...
`nautilus/core/broker.py:3104-3107`. `context["session_token"]` was present but not a `str`
...
`nautilus/core/broker.py:3111-3117`. The token is valid, but the request's `agent_id` is not the
...
`.revoke_key` (`nautilus/core/broker.py:1464`, `:1430`, `:1471`, `:1505`).
...
`nautilus/core/broker.py:1468-1472`. The requested `purpose` is not in that agent's
...
- `handoff requires the originating agent's session token` — `nautilus/core/broker.py:2008`.
- `session token rejected: {exc.reason_code}` — `nautilus/core/broker.py:2031`, using the same
