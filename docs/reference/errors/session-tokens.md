...
**HTTP 401.** `nautilus/transport/auth.py:302-305` and `:315-318`, from the
...
**HTTP 401**, with `WWW-Authenticate: Bearer`. `nautilus/transport/fastapi_app.py:713-719`. The
...
`nautilus/core/broker.py:3011-3014`. `context["session_token"]` was present but not a `str`
...
`nautilus/core/broker.py:3018-3024`. The token is valid, but the request's `agent_id` is not the
...
`.revoke_key` (`nautilus/core/broker.py:1436`, `:1430`, `:1471`, `:1505`).
...
`nautilus/core/broker.py:1440-1444`. The requested `purpose` is not in that agent's
...
- `handoff requires the originating agent's session token` — `nautilus/core/broker.py:1980`.
- `session token rejected: {exc.reason_code}` — `nautilus/core/broker.py:2003`, using the same
