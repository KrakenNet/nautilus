...
(`nautilus/core/broker.py:1685`) when the broker builds the adapter.
...
**`ConfigError`**, `nautilus/core/broker.py:1471-1475`. `analysis.mode: llm` needs a provider
...
**`ConfigError`**, `nautilus/core/broker.py:690-695`. `{known}` is the configured hierarchy;
...
Raised by `Broker._load_custom_adapters` (`nautilus/core/broker.py:404-449`). `{i}` is the index
...
**`ConfigError`**, `nautilus/core/broker.py:1692-1697`. The source type is built in, but its
