...
(`nautilus/transport/fastapi_app.py:185-191`, one `log.warning` line.)
...
**HTTP 403.** `nautilus/transport/fastapi_app.py:662-669`. The MCP transport raises the same
...
**HTTP 403.** `nautilus/transport/fastapi_app.py:935-942`, on `POST /v1/sessions`.
...
**HTTP 400.** `_require_reviewer`, `nautilus/transport/fastapi_app.py:1250-1256`.
...
`nautilus/transport/fastapi_app.py:194-201`. The index and the pluralisation change with the
